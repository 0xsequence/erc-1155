...
import { ERC1155MetaMintBurnPackedBalanceMock, ERC1155ReceiverMock, ProxyUpgradeableDeployerMock } from 'src'

// init test wallets from package.json mnemonic
import { web3 } from 'hardhat'

const { wallet: ownerWallet, provider: ownerProvider } = utils.createTestWallet(web3, 1)
const { wallet: receiverWallet } = utils.createTestWallet(web3, 2)
const { wallet: anyoneWallet } = utils.createTestWallet(web3, 3)
const { wallet: operatorWallet } = utils.createTestWallet(web3, 4)


const usingUpgradeable = [false, true]

usingUpgradeable.forEach(upgradeable => {
  describe('ERC1155MintBurnPackedBalance' + (upgradeable ? 'Upgradeable': ''), () => {
    ...
    context('When ERC1155MintBurn contract is deployed', () => {
      before(async () => {
        ...
        if (upgradeable) {
          abstract = await AbstractContract.fromArtifactName('ERC1155MetaMintBurnPackedBalanceUpgradeableMock')
          // Create factory
          ...
          abstract = await AbstractContract.fromArtifactName('ERC1155MetaMintBurnPackedBalanceMock')
        }
      })
      ...
      beforeEach(async () => {
        ...
        if (upgradeable) {
          ...
          // Create proxy
          let tx = factoryContract.createProxy(erc1155MintBurnContract.address, constants.HashZero, ownerWallet.address);
          await expect(tx).to.be.fulfilled
          const proxyAddr = await factoryContract.predictProxyAddress(erc1155MintBurnContract.address, constants.HashZero, ownerWallet.address);
          erc1155MintBurnContract = (await abstract.connect(ownerWallet, proxyAddr)) as ERC1155MetaMintBurnPackedBalanceMock;
          tx = erc1155MintBurnContract.initialize(NAME, METADATA_URI)
          await expect(tx).to.be.fulfilled
        } else {
      ...
      describe('_mint() function', () => {
        ...
        it('should REVERT if amount is larger than limit (invalid amount min overflow)', async () => {
          ...
          const maxVal2 = BigNumber.from(2)
            .pow(256)
            .sub(maxVal.sub(1))
          const tx = erc1155MintBurnContract.mintMock(anyoneWallet.address, 0, maxVal2, [], HIGH_GAS_LIMIT)
        ...
        it('should REVERT if amount is larger than limit (invalid amount max overflow)', async () => {
          ...
          const maxVal = BigNumber.from(2)
            .pow(256)
            .sub(1)
          const tx = erc1155MintBurnContract.mintMock(anyoneWallet.address, 0, maxVal, [], HIGH_GAS_LIMIT)
          await expect(tx).to.be.rejectedWith(RevertError('ERC1155PackedBalance#_viewUpdateBinValue: OVERFLOW'))
        })
      ...
      describe('_batchMint() function', () => {
        ...
        it('should REVERT if amount is larger than limit (invalid amount min overflow)', async () => {
          ...
          const maxVal2 = BigNumber.from(2)
            .pow(256)
            .sub(maxVal.sub(1))
          const tx3 = erc1155MintBurnContract.batchMintMock(anyoneWallet.address, [0], [maxVal2], [], HIGH_GAS_LIMIT)
          await expect(tx3).to.be.rejectedWith(RevertUnsafeMathError())
        })
        ...
        it('should REVERT if amount is larger than limit (invalid amount max overflow)', async () => {
          ...
          const maxVal3 = BigNumber.from(2)
            .pow(256)
            .sub(1)
          const tx4 = erc1155MintBurnContract.batchMintMock(anyoneWallet.address, [0], [maxVal3], [], HIGH_GAS_LIMIT)
          await expect(tx4).to.be.rejectedWith(RevertUnsafeMathError())
        })
      ...
      describe('_batchBurn() function', () => {
        ...
        it('should REVERT if amount is higher than balance', async () => {
          ...
          const tx = erc1155MintBurnContract.batchBurnMock(receiverAddress, [typesArray[0]], [invalidVal], HIGH_GAS_LIMIT)
