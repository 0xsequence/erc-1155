  ...
  context('When ERC1155MintBurn contract is deployed', () => {
    ...
    describe('_mint() function', () => {
      ...
      it('should REVERT if amount is larger than limit (overflow 1)', async () => {
        await erc1155MintBurnContract.mintMock(receiverAddress, tokenID, amount, [])
        const maxVal0 = BigNumber.from(2).pow(32).sub(amount)
      ...
      it('should REVERT if amount is larger than limit(invalid amount min overflow)', async () => {
        ...
        // Minimum overflow
        const maxVal2 = (BigNumber.from(2).pow(256)).sub(maxVal.sub(1))
      ...
      it('should have TransferSingle event emitted before onERC1155Received is called', async () => {
        // Get event filter to get internal tx event
        let tx = await erc1155MintBurnContract.mintMock(receiverContract.address, tokenID, amount, [])
        const receipt = await tx.wait(1)
  
        const firstEventTopic = receipt.logs![0].topics[0]
        const secondEventTopic = receipt.logs![1].topics[0]
  
        expect(firstEventTopic).to.be.equal(erc1155MintBurnContract.interface.getEventTopic(erc1155MintBurnContract.interface.events["TransferSingle(address,address,address,uint256,uint256)"]))
    ...
    describe('_batchMint() function', () => {
      ...
      it('should ALLOW inheriting contract to call _batchMint()', async () => {        
        let req = erc1155MintBurnContract.batchMintMock(receiverAddress, typesArray, amountArray, [])
      ...
      it('should REVERT if amount is larger than limit (overflow 1)', async () => {
        ...
        // Overflow by 1
        const maxVal0 = BigNumber.from(2).pow(32).sub(amountToMint)
        const tx0 = erc1155MintBurnContract.batchMintMock(receiverAddress, [typesArray[0], typesArray[1]], [maxVal0, amountArray[1]], [])
        await expect(tx0).to.be.rejectedWith( RevertError("ERC1155PackedBalance#_viewUpdateBinValue: OVERFLOW") )
      })
      ...
      it('should REVERT if amount is larger than limit (invalid amount max overflow)', async () => {
        ...
        await expect(balance).to.be.eql(maxVal.sub(1));
        const maxVal3 = (BigNumber.from(2).pow(256)).sub(1)
      ...
      it('should have balances updated before onERC1155BatchReceived is called', async () => {
        ...
        await erc1155MintBurnContract.batchMintMock(receiverContract.address, typesArray, amountArray, [], 
          {gasLimit: 2000000}
        )
      ...
      it('should have TransferBatch event emitted before onERC1155BatchReceived is called', async () => {
        // Get event filter to get internal tx event
        let tx = await erc1155MintBurnContract.batchMintMock(receiverContract.address, typesArray, amountArray, [],
          {gasLimit: 2000000}
        )
        const receipt = await tx.wait(1)
        ...
        expect(firstEventTopic).to.be.equal(erc1155MintBurnContract.interface.getEventTopic(erc1155MintBurnContract.interface.events["TransferBatch(address,address,address,uint256[],uint256[])"]))
        expect(secondEventTopic).to.be.equal(erc1155MintBurnContract.interface.getEventTopic(receiverContract.interface.events["TransferBatchReceiver(address,address,uint256[],uint256[])"]))
      })
    ...
    describe('_batchBurn() function', () => {
      ...
      it('should ALLOW inheriting contract to call _batchBurn()', async () => {        
        let req = erc1155MintBurnContract.batchBurnMock(receiverAddress, typesArray, burnAmountArray)
