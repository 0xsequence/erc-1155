...
const { wallet: anyoneWallet, signer: anyoneSigner } = utils.createTestWallet(web3, 3)

const usingUpgradeable = [false, true]

usingUpgradeable.forEach(upgradeable => {
  describe('ERC1155MintBurn' + (upgradeable ? 'Upgradeable': ''), () => {
    ...
    context('When ERC1155MintBurn contract is deployed', () => {
      before(async () => {
        ...
        if (upgradeable) {
          abstract = await AbstractContract.fromArtifactName('ERC1155MetaMintBurnUpgradeableMock')
          // Create factory
          ...
          abstract = await AbstractContract.fromArtifactName('ERC1155MetaMintBurnMock')
        }
      })
      ...
      beforeEach(async () => {
        ...
        if (upgradeable) {
          ...
          erc1155MintBurnContract = (await abstract.connect(ownerWallet, proxyAddr)) as ERC1155MetaMintBurnMock;
