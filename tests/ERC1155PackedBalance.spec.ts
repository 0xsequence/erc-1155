...
import { ERC1155MetaMintBurnPackedBalanceMock, ERC1155ReceiverMock, ERC1155OperatorMock, ProxyUpgradeableDeployerMock } from 'src'
...
usingUpgradeable.forEach(upgradeable => {
  describe('ERC1155PackedBalance' + (upgradeable ? 'Upgradeable': ''), () => {
    ...
    beforeEach(async () => {
      if (upgradeable) {
        ...
        // Create proxy
        let tx = factoryContract.createProxy(erc1155Contract.address, constants.HashZero, ownerWallet.address);
        await expect(tx).to.be.fulfilled
        const proxyAddr = await factoryContract.predictProxyAddress(erc1155Contract.address, constants.HashZero, ownerWallet.address);
        erc1155Contract = (await erc1155Abstract.connect(ownerWallet, proxyAddr)) as ERC1155MetaMintBurnPackedBalanceMock;
