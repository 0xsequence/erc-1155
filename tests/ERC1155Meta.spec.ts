...
import { BigNumber, utils, ContractTransaction, EventFilter, constants } from 'ethers'
import { ERC1155MetaMintBurnMock, ERC1271WalletValidationMock, ERC1155ReceiverMock, ERC1155OperatorMock, ERC20Mock, ProxyUpgradeableDeployerMock } from 'src'
import { GasReceipt, TransferSignature, ApprovalSignature, BatchTransferSignature } from 'src/typings/tx-types'

// init test wallets from package.json mnemonic
import { web3 } from 'hardhat'

const { wallet: ownerWallet, provider: ownerProvider } = createTestWallet(web3, 1)
...
usingUpgradeable.forEach(upgradeable => {
  describe('ERC1155Meta' + (upgradeable ? 'Upgradeable': ''), () => {
    ...
    const deployERC1155Contract = async () => {
      let contract: ERC1155MetaMintBurnMock
      if (upgradeable) {
        contract = (await erc1155Abstract.deploy(ownerWallet, [])) as ERC1155MetaMintBurnMock

        // Create proxy
        let tx = factoryContract.createProxy(contract.address, constants.HashZero, ownerWallet.address);
        await expect(tx).to.be.fulfilled
        const proxyAddr = await factoryContract.predictProxyAddress(contract.address, constants.HashZero, ownerWallet.address);
        contract = (await erc1155Abstract.connect(ownerWallet, proxyAddr)) as ERC1155MetaMintBurnMock;
        tx = contract.initialize(NAME, METADATA_URI)
        await expect(tx).to.be.fulfilled
      } else {
        contract = (await erc1155Abstract.deploy(ownerWallet, [NAME, METADATA_URI])) as ERC1155MetaMintBurnMock
      }
      return contract
    }
    ...
    beforeEach(async () => {
      erc1155Contract = await deployERC1155Contract()
      ...
      conditions.forEach(function(condition) {
        context(condition[2] as string, () => {
          ...
          describe('When signature is valid', () => {
            ...
            it.skip('should REVERT if invalid response from receiver contract', async () => {
            ...
            describe('When gas is reimbursed', () => {
              ...
              it('should PASS if another approved ERC-1155 is used for fee', async () => {
                const erc1155Contract2 = await deployERC1155Contract()
