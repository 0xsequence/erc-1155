    ...
    c(`With ${bits} bits for values`, () => {
      context('When ERC1155MintBurn contract is deployed', () => {
        ...
        beforeEach(async () => {
          ...
          erc1155MintBurnContract = (await abstract.deploy(ownerWallet, [bits])) as ERC1155MetaMintBurnPackedBalanceMock
        ...
        describe('_mint() function', () => {
          ...
          it('should REVERT if amount is larger than limit (invalid amount by 1)', async () => {
            const maxVal = BigNumber.from(2).pow(bits)
        ...
        describe('_batchMint() function', () => {
          const Ntypes = bits < 128 ? 123 : 25
          const amountToMint = bits > 4 ? 10 : 1
        ...
        describe('_burn() function', () => {
          const tokenID = 666
          const initBalance = bits > 4 ? 10 : 1
          const amountToBurn = bits > 4 ? 10 : 1
        ...
        describe('_batchBurn() function', () => {
          const Ntypes = 32
          const initBalance = bits > 4 ? 100 : 1
          const amountToBurn = bits > 4 ? 30 : 1
          const typesArray = Array.apply(null, { length: Ntypes }).map(Number.call, Number)
