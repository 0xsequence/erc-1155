...
usingUpgradeable.forEach(upgradeable => {
  describe('ERC1155MetaPackedBalance' + (upgradeable ? 'Upgradeable': ''), () => {
    ...
    before(async () => {
      ...
      if (upgradeable) {
        erc1155Abstract = await AbstractContract.fromArtifactName('ERC1155MetaMintBurnPackedBalanceUpgradeableMock')
        // Create factory
    ...
    const deployERC1155Contract = async () => {
      let contract: ERC1155MetaMintBurnPackedBalanceMock
      if (upgradeable) {
        contract = (await erc1155Abstract.deploy(ownerWallet, [])) as ERC1155MetaMintBurnPackedBalanceMock
        ...
        contract = (await erc1155Abstract.connect(ownerWallet, proxyAddr)) as ERC1155MetaMintBurnPackedBalanceMock;
        tx = contract.initialize(NAME, METADATA_URI)
        await expect(tx).to.be.fulfilled
      } else {
        contract = (await erc1155Abstract.deploy(ownerWallet, [NAME, METADATA_URI])) as ERC1155MetaMintBurnPackedBalanceMock
      }
      return contract
    }
