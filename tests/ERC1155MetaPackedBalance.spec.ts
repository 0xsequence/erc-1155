...
function valFor(bits, tier) {
  switch (bits) {
    case 1: return 1
    case 2: return [1, 2, 3][tier]
    case 4: return [2, 5, 11][tier]
    case 8: return [2, 100, 200][tier]
    case 16: return [2, 520, 1100][tier]
  }

  return [50, 5000, 15000][tier]
}
...
describe('ERC1155MetaPackedBalance', () => {
  ...
  VALID_ID_BITS.map((bits) => {
    ...
    context(`With ${bits} bits for values`, () => {
      ...
      beforeEach(async () => {
        erc1155Contract = (await erc1155Abstract.deploy(ownerWallet, [bits])) as ERC1155MetaMintBurnPackedBalanceMock
      ...
      describe('metaSafeTransferFrom() (Meta) Function', () => {
        ...
        const initBalance = valFor(bits, 2)
        const amount = valFor(bits, 0)
        const nonce = BigNumber.from(0)
        const id = 66

        const feeTokenID = 666
        let isGasReceipt: boolean = true
        const feeTokenInitBalance = BigNumber.from(valFor(bits, 2))
      ...
      describe('metaSetApprovalForAll() function', () => {
        const initBalance = valFor(bits, 1)
